 ...
 */
    ...
    public void cookieTest() {
        ...
        //2-tum cookie'leri listeleyin(1.yol)
        Set<Cookie> allCookies = driver.manage().getCookies();
        System.out.println("All cookies = " + allCookies);

        // for each le alt alta listeleyebilirim(2.yol)
        for (Cookie w : allCookies) {
            System.out.println(w.getName() + w.getExpiry());
        }
       // lambda ile;
        allCookies.stream().forEach(t->System.out.println(t));// (3.yol)


        //3-Sayfadaki cookies sayisinin 5’den buyuk oldugunu test edin

        int cookiesNumber = allCookies.size();
        System.out.println("Cookie Number=" + cookiesNumber);
